    ...
    if MODE == 0:
        for job in jobs:
            ...
            if len(bind_job) > 1:
                ...
                bind_raw.append(tmp_add-time_machine[count*USE_MACHINE+int(bind_job[0])-1])
                contens[count*USE_MACHINE+int(bind_job[0])-1][handle_machine[count*USE_MACHINE+int(bind_job[0])-1]]=tmp_add
...
def demo():
    if request.method == 'GET':
        ...
        PSO.PSO_RANDOM(jobs, USE_MACHINE, MAX_MACHINE, bind_array, contens,handle_machine,time_machine)
        return render_template('draw_pso.html')
